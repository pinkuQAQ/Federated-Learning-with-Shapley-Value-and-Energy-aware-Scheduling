...
\item We validate the proposed scheduler on CIFAR-10 through ablation, heterogeneity sensitivity, sampling-budget sensitivity, and channel-noise diagnostic analysis, and we add Fashion-MNIST and MNIST checks to examine whether the same scheduling behavior transfers across datasets.
\end{enumerate}
...
The released aggregate is perturbed by an equivalent layer-wise channel-noise term:
\begin{equation}
\label{eq:ch_noise}
\begin{aligned}
\tilde{\Delta w}_{\ell}(t)
&=\bar{\Delta w}_{\ell}(t)+z_{\mathrm{ch},\ell}(t),\\
z_{\mathrm{ch},\ell}(t)
&\sim \mathcal{N}\!\left(
0,\frac{\sigma_{\mathrm{eq},t}^2C_{\ell,t}^2}{K_t^2}I\right).
\end{aligned}
\end{equation}
where $K_t=|\mathcal{S}_t|$ is the number of clients participating in round $t$, $C_{\ell,t}$ is the adaptive clipping threshold of layer $\ell$, and $\sigma_{\mathrm{eq},t}$ controls the equivalent channel-noise multiplier. The factor $1/K_t$ provides a client-count-normalized perturbation scale that is applied consistently across scheduling methods. Because the simulator uses data-size-weighted aggregation, this diagnostic scale is not presented as a formal sensitivity bound or differential-privacy guarantee.
...
After the selected clients finish local training, the server receives their updates and enters the state-refresh stage. First, each returned update is clipped by the adaptive threshold described in Section~\ref{sec:system}. Then the server forms the weighted aggregate and applies the channel-noise diagnostic to the released aggregate. For each layer $\ell$, the diagnostic standard deviation is $\sigma_{\mathrm{eq},t}C_{\ell,t}/K_t$, independently of the realized data-size aggregation weights. The actual maximum aggregation weight is recorded for analysis, but it does not determine the diagnostic noise magnitude.
...
Table~\ref{tab:main} reports the main CIFAR-10 scheduling results and the additional Fashion-MNIST/MNIST checks under $\alpha=0.1$ and $T=100$, averaged over three seeds. All methods use the same local training protocol, data partition, energy model, and channel-noise diagnostic setting within each dataset. Therefore, the table mainly reflects the difference of scheduling rules under matched perturbation protocols.

\begin{table}[t]
\centering
\caption{Accuracy results across datasets.}
\label{tab:main}
\scriptsize
\setlength{\tabcolsep}{2.2pt}
\begin{tabular}{l|l|c|c}
\hline
\textbf{Dataset} & \textbf{Method} & \textbf{Last-5 acc.\ (\%)} & \textbf{Final acc.\ (\%)} \\
\hline
CIFAR-10 & Ours    & \textbf{32.35 $\pm$ 1.47} & \textbf{35.49 $\pm$ 3.88} \\
...
\hline
Fashion-MNIST & Ours    & \textbf{82.20 $\pm$ 0.53} & \textbf{83.86 $\pm$ 0.47} \\
              & FedAvg  & 78.97 $\pm$ 1.06 & 80.13 $\pm$ 1.99 \\
              & FedProx & 79.03 $\pm$ 0.92 & 80.15 $\pm$ 1.76 \\
              & Oort    & 79.80 $\pm$ 0.58 & 79.69 $\pm$ 0.34 \\
              & GCA     & 79.58 $\pm$ 1.65 & 78.04 $\pm$ 6.57 \\
\hline
MNIST    & Ours    & 90.92 $\pm$ 1.37 & 92.09 $\pm$ 1.13 \\
         & FedAvg  & 91.28 $\pm$ 0.83 & 91.33 $\pm$ 1.58 \\
         & FedProx & 91.22 $\pm$ 0.86 & 91.25 $\pm$ 1.75 \\
         & Oort    & 89.76 $\pm$ 2.94 & 90.18 $\pm$ 2.79 \\
         & GCA     & \textbf{92.32 $\pm$ 1.57} & \textbf{93.00 $\pm$ 1.47} \\
\hline
\end{tabular}
\end{table}
...
On CIFAR-10, Ours improves the last-5-round accuracy over FedAvg by 6.01 percentage points and over the stronger GCA baseline by 0.48 percentage points. The gain over GCA is not large in last-5 accuracy, but it is larger in final accuracy. On Fashion-MNIST, the proposed scheduler obtains the highest final and last-5-round accuracy, with smaller variance than the random-selection baselines. On MNIST, GCA gives the highest mean accuracy, while the proposed method remains competitive with FedAvg and FedProx and above Oort. This suggests that the Shapley-guided queue scheduler is effective on CIFAR-10 and Fashion-MNIST, while its relative advantage can depend on dataset difficulty and on how informative the loss/channel proxy used by GCA is in a simpler classification task.
...
The channel-noise component makes the simulations closer to wireless FL settings, where the received aggregate may be affected by natural propagation noise. Related wireless privacy studies show that channel noise can be incorporated into privacy-aware aggregation models when the physical-layer transmission and privacy budget are jointly considered \cite{yan2023otafedavgprivacy}. In this paper, all compared schedulers use the same nominal channel-noise multiplier and the same $1/K_t$ scaling rule. The realized layer-wise scale can still vary with the adaptive clipping threshold. This diagnostic is used for noise--utility evaluation and is not claimed as a formal differential-privacy guarantee.
