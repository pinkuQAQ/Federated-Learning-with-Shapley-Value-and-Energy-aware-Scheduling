...
def load_record(path, run_root):
    ...
    noise_scale_weights = np.asarray(
        [as_float(item.get("noise_scale_weight")) for item in aggregate_records],
        dtype=np.float64,
    )
    ...
    return {
        ...
        "mean_noise_scale_weight": as_float(np.nanmean(noise_scale_weights))
        if noise_scale_weights.size else np.nan,
